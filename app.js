...
const metrics = [
  ...
  ["±15 В", "внешнее двухполярное питание, без сетевых цепей внутри корпуса"],
...
const specs = [
  ...
  ["Питание", "внешний двухполярный источник ±15 В DC с ограничением тока"],
...
const operations = [
  ...
  ["060", "Контрольно-измерительная", "Подать питание ±15 В, проконтролировать отсутствие КЗ, перегрева и клиппинга в рабочем режиме.", "transistors"],
];
...
const sections = [
  ...
  },
  {
    ...
    focus: "connectors",
    render: () => `
      <p class="lead">Внутри корпуса отсутствуют цепи 220 В: питание подаётся от внешнего двухполярного источника ±15 В. Основные риски связаны с коротким замыканием, перегревом силовых элементов и механическим повреждением соединений.</p>
      <div class="grid">
        <div class="doc-box"><h3>Электробезопасность</h3><ul class="plain-list"><li>использовать только внешний источник ±15 В с ограничением тока;</li><li>не включать изделие при снятой крышке;</li><li>исключить попадание металлических предметов через вентиляцию;</li><li>маркировать полярность разъёма питания.</li></ul></div>
